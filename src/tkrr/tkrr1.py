...
## Print out errors and diagnostics.
#print " TRAIN STD   = %.4f"%intrins_dev1
#print " TRAIN RMSE  = %.4f %%"%intrins_error1
...
## Print out errors and diagnostics.
#print "--------------------------------"
#print " TEST STD  = %.4f"%intrins_dev1
#print " TEST RMSE = %.4f %%"%intrins_error1
#
## Print results for vector moduli
#avdip = 0.0
#for i in range(ns):
#    avdip += np.linalg.norm(vtest1.reshape((ns,3))[i])
#avdip /= float(ns)
#stdip = 0.0
#for i in range(ns):
#    stdip += (np.linalg.norm(vtest1.reshape((ns,3))[i])-avdip)**2
#stdip = np.sqrt(stdip/float(ns))
#abs_error = 0.0
#predi = predcart.reshape((ns,3))
#for i in range(ns):
#    abs_error += (np.linalg.norm(vtest1.reshape((ns,3))[i])-np.linalg.norm(outvec1.reshape((ns,3))[i]))**2
#abs_error = np.sqrt(abs_error/float(ns))
